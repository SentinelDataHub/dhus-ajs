...
angular
	...
	.factory('SearchModel', function (Logger, StyleService) {
		...
		var service = {
			...
			/*
			* Function called by two directives
			*/
			OnSelectionChange: function () {
				var self = this;
				return self.counterSelected();
			},

			//SET Selected Products function
			setOnSelectionChange: function (method) {
				this.OnSelectionChange = method;
			},

			//Returns number of currently selected products
			countSelected: function () {
				var self = this;
				var counterSelected = 0;

				if (self.model.list != undefined) {
					for (var i = 0; i < self.model.list.length; i++) {
						if (self.model.list[i].selected) {
			...
			selectSingleProduct: function (param) {
				console.warn('160 searchModelService WARNING! DELETED selectSingleProduct, reactivate if you see this message', param);
				// var self = this;
			...
			deselectProduct: function (param) {
				var self = this;
				var index = this.getProductIndexByUUID(param.uuid);
				if (self.model.list != undefined && self.model.list[index] != undefined) {
			...
			nohighlightProduct: function (param) {
				// console.warn('191 search-model-service nohighlightProduct: ', param);
				// var index = this.getProductIndexByUUID(param.uuid);
			...
			getSelectedStyle: function (productStyle) {
				// console.warn('191 search-model-service getSelectedStyle: ', param);
				var style;
